		...
		stage('Update the Database'){
			steps{
				script{
					...
					def response = httpRequest authentication: 'jenkinssbCredentials', httpMode:'POST' , validResponseCodes='405' , url: "http://192.168.0.101:8081/locationsUpdate/${primaryKey}/${updatedValue}"
				}
			}
		}
    }
